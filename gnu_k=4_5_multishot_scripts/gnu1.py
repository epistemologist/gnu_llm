...
def is_gnu4(factorization: dict) -> bool:
    ...
    # Case 4: n = p1^2 * p2^2 * ..., all groups are abelian (no congruences)
    # Exponents are 1 or 2, with at least two 2s.
    ...
    if len(primes_with_sq) == 1 and factorization[primes_with_sq[0]] == 2 and all(e == 1 for p, e in factorization.items() if p not in primes_with_sq):
        ...
        if total_congs == 1:
            ...
            q_cong = list(graph[p_cong])[0]
            if q_cong != p1:
                # The condition "not any(p % p1 == 1 for p in p_rest)" is implied by total_congs=1 and q_cong!=p1
                # Text says "none of these numbers divides p1^2-1", code implements p1+1.
                # The logic seems to be to check for conditions that would create non-abelian groups of order p1^2*p.
                if not any((p1 * p1 - 1) % p == 0 for p in p_rest if p != q_cong):
                     return True
