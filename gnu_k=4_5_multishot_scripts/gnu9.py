...
def is_gnu5(factorization: dict) -> bool:
    ...
    if all(e == 1 for e in exponents):
        ...
        # Case SF2: "Former case". A structure with 3 congruences.
        if total_congs == 3:
            # First condition from paper: largest prime with an outgoing edge cannot have out-degree >= 2.
            ...
            # The structure for this case requires a prime with out-degree 2,
            # but no prime with in-degree 2.
            # The buggy code checked for (indegree_2 AND outdegree_2), which was incorrect.
            # E.g., for n=17205={3,5,31,37}, graph is {31->{3,5}, 37->{3}}.
            # It has out(31)=2 AND in(3)=2, so it should be False for gnu=5.
            has_indegree_2 = any(len(p_set) == 2 for p_set in inv_graph.values())
            has_outdegree_2 = any(len(p_set) == 2 for p_set in graph.values())
            
            if has_outdegree_2 and not has_indegree_2:
                return True
