...
def is_gnu5(factorization: dict) -> bool:
    ...
    if all(e == 1 for e in exponents):
        ...
        # Case SF2/3: The complex case from the paper.
        # This case requires exactly 3 congruences in total.
        if total_congs != 3:
            return False

        # It also requires a condition on the largest prime that has a congruence.
        # This condition, from the first sentence on gnu=5 cases in the text, was
        # missed in the original implementation and is the key to fixing the bug.
        # "the largest prime factor of g which is congruent to unity ... 
        #  can obviously not be thus congruent with respect to two such factors"
        p_cong_list = sorted(list(graph.keys()), reverse=True)
        if not p_cong_list: return False # Should not be reached if total_congs==3
        
        p_max_cong = p_cong_list[0]
        if len(graph[p_max_cong]) >= 2:
            return False

        # If the conditions above hold, the structure must be:
        # p_i = 1 (mod p_k) and p_j = 1 (mod p_k) for some p_k,
        # plus one other congruence.
        # This is equivalent to some prime having an in-degree of 2 in the inv_graph.
        if any(len(p_set) == 2 for p_set in inv_graph.values()):
            return True
            ...
            if q_cong == p1:
                ...
                if (p_cong - 1) % (p1 * p1) == 0:
                    ...
                    if not any((p1 * p1 - 1) % pr == 0 for pr in p_rest if pr != p_cong):
                        return True
                # Case SQ2: p is not 1 mod p1^2, and the p1+1 condition for gnu=4 fails.
                else: 
                    # This case corresponds to gnu=4, Case 5, but with its last condition failing.
                    # gnu=4 requires (p1+1) is NOT divisible by any p in p_rest.
                    # gnu=5 occurs if (p1+1) IS divisible by some p in p_rest.
