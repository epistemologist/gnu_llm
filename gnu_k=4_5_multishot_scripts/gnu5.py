        ...
        if total_congs == 1:
            ...
            if q_cong == p1:
                ...
                if q_special % (p1 * p1) != 1:
                    ...
                    # "other prime factors" here refers to all primes in p_rest (p != p1), including q_special.
                    # This interpretation is crucial for distinguishing n=12 (gnu=5) from n=28 (gnu=4).
                    # For n=12=2^2*3: p1=2, p_rest={3}, q_special=3. p1+1=3. 3 divides 3. Condition fails.
                    # For n=28=2^2*7: p1=2, p_rest={7}, q_special=7. p1+1=3. 7 does not divide 3. Condition passes.
...
def is_gnu5(factorization: dict) -> bool:
    ...
    if all(e == 1 for e in exponents):
        ...
        if len(primes) >= 4:
            graph = get_congruence_graph(primes)
            total_congs_direct = sum(len(v) for v in graph.values())
            for p1 in primes:
    ...
    if len(primes_with_cube) == 1 and factorization[primes_with_cube[0]] == 3 and all(e == 1 for p_other, e in factorization.items() if p_other != primes_with_cube[0]):
        ...
        if not any(graph.values()):
            ...
            if p1 == 2:
                ...
                aut_prime_factors = set()
                # From Aut(Z_p1^3) -> p1^2(p1-1)
                # From Aut(Z_p1^2 x Z_p1) -> p1^3(p1-1)^2(p1+1)
                # From Aut((Z_p1)^3) -> p1^3(p1-1)(p1^2-1)(p1^3-1) = p1^3(p1-1)^2(p1+1)(p1^2+p1+1)
                # We need to check for prime factors of p1-1, p1+1, p1^2+p1+1
    ...
    if len(primes_with_sq) == 1 and factorization[primes_with_sq[0]] == 2 and all(e == 1 for p_other, e in factorization.items() if p_other != primes_with_sq[0]):
        ...
        if total_congs == 1:
            ...
            q_cong = list(q_cong_set)[0]
            
            if q_cong == p1 and p_cong % (p1 * p1) == 1:
        ...
        if total_congs == 0:
            ...
            if len(divisors_of_p1_plus_1) == 1:
                ...
                # The condition is that q-1 is divisible by one and only one SUCH factor,
