...
def is_gnu5(factorization: dict) -> bool:
    ...
    if all(e == 1 for e in exponents):
        ...
        # Case SF1: "Latter case". Two congruences, both of the form p=1(mod 3).
        # Graphically: p1 -> 3, p2 -> 3. Total congruences must be 2.
        if 3 in primes and len(inv_graph.get(3, [])) == 2 and total_congs == 2:
            return True

        # Cases SF2/SF3: "Former case". A structure with 3 congruences.
        if total_congs == 3:
            # Condition from paper: "largest prime factor ... cannot be ... congruent with respect to two such factors."
            p_cong_list = list(graph.keys())
            if not p_cong_list: return False # Should be unreachable

            if len(graph[max(p_cong_list)]) >= 2:
                return False

            # The two structures for gnu=5 with 3 congruences are:
            # A) A "fork": p1->p2, p2->{p3,p4}. Max in-degree is 1. One node has out-degree 2.
            # B) A "path": p1->p2->p3->p4. Max in-degree is 1. Max out-degree is 1.
            # A common necessary condition is that no prime has an in-degree > 1.
            # This check, combined with total_congs=3 and the largest prime rule, covers both cases.
            has_indegree_gt_1 = any(len(p_set) > 1 for p_set in inv_graph.values())
            if not has_indegree_gt_1:
                return True
