...
def is_gnu5(factorization: dict) -> bool:
    ...
    if all(e == 1 for e in exponents):
        ...
        # Cases SF2/SF3: "Former case". A connected structure with 3 congruences (a path or a fork).
        if total_congs == 3:
            # Condition 1: "largest prime factor ... cannot be ... congruent with respect to two such factors."
            p_cong_list = list(graph.keys())
            if not p_cong_list: return False # Should be unreachable

            if len(graph.get(max(p_cong_list), set())) >= 2:
                return False

            # Condition 2: No prime can be the target of more than one congruence (in-degree <= 1).
            if any(len(p_set) > 1 for p_set in inv_graph.values()):
                return False

            # Condition 3: The graph of congruences must be connected.
            # This distinguishes the required path/fork structures from disconnected congruences.
            nodes = set(graph.keys()) | set(inv_graph.keys())
